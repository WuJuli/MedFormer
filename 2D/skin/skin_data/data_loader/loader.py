...
class weak_annotation(torch.nn.Module):
    def __init__(self, patch_size=16, img_size=256):
        super().__init__()
        self.arranger = Rearrange('c (ph h) (pw w) -> c (ph pw) h w', c=1, h=patch_size, ph=img_size // patch_size,
                                  w=patch_size, pw=img_size // patch_size)
...
class isic_loader(Dataset):
    ...
    def __init__(self, path_Data, train=True, Test=False):
        ...
        if '2018' in path_Data:
            ...
            if train:
                self.data = np.load(path_Data + 'data_train_2017.npy')
                self.mask = np.load(path_Data + 'mask_train_2017.npy')
            else:
                if Test:
                    self.data = np.load(path_Data + 'data_test_2017.npy')
                    self.mask = np.load(path_Data + 'mask_test_2017.npy')
                else:
                    self.data = np.load(path_Data + 'data_val_2017.npy')
                    self.mask = np.load(path_Data + 'mask_val_2017.npy')
